<!DOCTYPE html>
<html>
<head>
	<link rel="icon" type="image/png" href="images/gannet-icon.png"/>
	<link rel = "stylesheet" href = "css/style.css"/>
	<link rel = "stylesheet" href = "css/menu.css"/>
		
	<title>Zulu Music</title>
		<meta charset="UTF-8">
		<meta name="Keywords" content="html5, layout, 10 Credit Project, Jake B"/>
		<meta name="Author" content="Jake Boyd"/>
		<meta name="Description" content="10 Credit Project - Jake Boyd">
		<meta name="viewport" content="width=device-width, initial-scale=1">
</head>
<body>
	<div class="menu-wrap"> <!-- https://www.youtube.com/watch?v=DZg6UfS5zYg Video used for burger menu --> 
		<input type="checkbox" class="toggler">
		<div class="hamburger"><div></div></div>
		<div class="menu">
			<div>
				<div>
					<ul>
						<li><a href="index.html">Home</a></li> <!-- Home page -->
						<li><a href="page2.html">Music by Title</a></li> <!-- Page with Music -->
						<li><a href="page3.html">Music by Genre</a></li> <!-- Page with Music 2 -->
						<li><a href="contact.php">Contact</a></li> <!-- Contact page -->
					</ul>
				</div>
			</div>
		</div>
	</div>
	
	<header class="showcase">
	<div class="container showcase-inner">
		<br><br><br><br><br><br><br><br><br><br>
		<h1>Z U L U</h1>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. 
			Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut.</p>
		<a href="page2.html" class="btn">Start Listening</a> <!-- Link to the music page (2nd page) -->
		<br><br><br><br><br><br><br><br><br><br>
		<div id="footer">
			&copy; Copyright Jake Boyd, all rights reserved, Tawa College 2022
		</div>
	</div>
	</header>
	
</body>
</html>